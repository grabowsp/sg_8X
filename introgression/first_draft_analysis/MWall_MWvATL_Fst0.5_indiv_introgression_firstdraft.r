...
### INPUT DATA ###
vcf_file <- '/home/f1p1/tmp/switchgrass_8X/firstdraft_vcfs/AtlanticvsMidwest.hiFst.firstdraft.Fst0.5.disomic.CDS.vcf'
...
pop_freq_file <- '/home/f1p1/tmp/switchgrass_8X/firstdraft_introg_results/MWall_freq_files/AtlanticvsMidwest.hiFst.firstdraft.Fst0.5.disomic.CDS.MWall.frq'
pop_freqs <- fread(pop_freq_file)
colnames(pop_freqs) <- c('CHROM', 'POS', 'N_ALLELES', 'N_CHR', 'ALLELE1_FREQ',
  'ALLELE2_FREQ')
pop_freqs$ID <- paste(pop_freqs$CHROM, pop_freqs$POS, sep = '_')

allele_state_file <- '/home/f1p1/tmp/switchgrass_8X/introgression_files/MW_v_ATL_Fst0.5_allele_states.txt'
...
keep_pos_file <- '/home/f1p1/tmp/switchgrass_8X/introgression_files/MW_v_ATL_Fst0.5_keep_pos.txt'
keep_pos <- fread(keep_pos_file)
...
out_dir <- '/home/f1p1/tmp/switchgrass_8X/firstdraft_introg_results/'
...
window_result_file_out <- paste(out_dir, testgrp_name, '_', pop2_name,
  '_into_', pop1_name, '_Fst_', Fst_val, '_', test_window_size, 
  'SNP_window', '_results.rds', sep = '')
...
#   and Redundancy
rda_snp_info_file_out <- paste(out_dir, testgrp_name, '_', pop2_name,
  '_into_', pop1_name, '_Fst_', Fst_val, '.RDA_SNP_info.txt', sep = '')
rda_geno_file_rds <- paste(out_dir, testgrp_name, '_', pop2_name,
  '_into_', pop1_name, '_Fst_', Fst_val, '.NPOP2alleles.rds', sep = '')

# plot files
tot_adj_n_pop2_plot_file <- paste(out_dir, testgrp_name, '_', pop2_name, 
  '_into_', pop1_name, '_Fst_', Fst_val, '_', test_window_size, 'SNP_window', 
  '.adj_n_pop2.pdf', sep = '')
...
samp_vec <- samp_info[grep('MW_', samp_info$subgrp_v2), samp_name]
#samp_vec <- colnames(vcf_in)[c(10:ncol(vcf_in))]

# Remove SNPs that aren't in the allele_state file
keep_ids <- paste(keep_pos$CHR, keep_pos$POS, sep = '_')
vcf_in <- vcf_in[vcf_in$ID %in% keep_ids, ]

pop_freqs <- pop_freqs[pop_freqs$ID %in% keep_ids, ]
...
if(sum(vcf_in$POS != allele_states$POS) > 0){
  print('Need to re-order ALLELE STATES table SNPs')
}
...
## Joe needs Rows = samples, columns = SNPs
rda_mat <- matrix(
  unlist(lapply(tot_pop2_list, function(x) x[['rda_geno_vec']])), byrow = T,
  nrow = length(tot_pop2_list))

rownames(rda_mat) <- names(tot_pop2_list)

snp_names <- paste(allele_states$CHR, allele_states$POS, sep = '_')
colnames(rda_mat) <- snp_names

af_1 <- as.numeric(unlist(
  lapply(strsplit(pop_freqs$ALLELE1_FREQ, split = ':'), function(x) x[2])))
af_2 <- as.numeric(unlist(
  lapply(strsplit(pop_freqs$ALLELE2_FREQ, split = ':'), function(x) x[2])))

af_tab <- data.table(af_1 = af_1, af_2 = af_2)
maf_vec <- apply(af_tab, 1, function(x) x[which.min(x)])
pop_freqs$MAF <- maf_vec

af_cutout_inds <- which(maf_vec < MAF_CUT)

num_pop2_allele <- apply(rda_mat, 2, sum)
no_pop2_allele_inds <- which(num_pop2_allele == 0)

rda_remove_inds <- union(af_cutout_inds, no_pop2_allele_inds)

rda_mat_1 <- rda_mat[ , -rda_remove_inds]

saveRDS(rda_mat_1, file = rda_geno_file_rds)

fwrite(pop_freqs[-rda_remove_inds, ], rda_snp_info_file_out, sep = '\t')

# Ends up being 24,004 SNPs
