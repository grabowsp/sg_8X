...
allele_distr_func_file <- 'FIGURE OUT FILE HERE'
...
for(gco in c(0.9, 0.95, 0.99)){
  ...
  # split into "ecotype"
  eco_inds <- list()
  eco_inds[['up']] <- which(admix_res$V4 > group_cutoff)
  eco_inds[['low']] <- which(admix_res$V4 < (1-group_cutoff))
  eco_group_list[[co_name]] <- eco_inds
  ###
  up_8X_inds <- intersect(which(admix_res$V1 %in% meta_8X_names),
    eco_inds[['up']])
...
samp_meta[VCF_NAME %in% admix_res$V1[eco_inds[['up']]], .N, by=NQUIRE_PLOIDY]
# 127 8X, 130 4X
samp_meta[VCF_NAME %in% admix_res$V1[eco_inds[['low']]], .N, by=NQUIRE_PLOIDY]
# 443 4X, 17 8X, 3 6X
