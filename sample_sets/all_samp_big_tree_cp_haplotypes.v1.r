...
weird_lib_file <- '/global/homes/g/grabowsp/data/switchgrass/first_draft_analysis/List_of_upland_cp_LIBs.csv'
weird_libs <- fread(weird_lib_file)

res_tab_file <- '/global/homes/g/grabowsp/data/switchgrass/results_tables_8X/natv2filt_res_tab_v4.1.txt'
res_tab <- fread(res_tab_file)

quest_lib_file <- '/global/homes/g/grabowsp/data/switchgrass/first_draft_analysis/natural_lines_with_strange_LIBs_June.csv'
quest_libs <- fread(quest_lib_file)
...
weird_samps <- samp_meta[LIB %in% weird_libs$LIB, VCF_NAME]
quest_samps <- samp_meta[LIB %in% quest_libs$LIB, VCF_NAME]

cp_hap_vec <- rep('NOT_INCLUDED', times = nrow(dist_mat))
cp_hap_vec[which(rownames(dist_mat) %in% 
  res_tab[, samp_name])] <- 'NOT_WEIRD_HAP'
cp_hap_vec[which(rownames(dist_mat) %in% weird_samps)] <- 'WEIRD_HAP'
cp_hap_vec[which(rownames(dist_mat) %in% quest_samps)] <- 'QUESTIONABLE'

cp_col_vec <- c()
cp_col_vec['NOT_INCLUDED'] <- 'grey80'
cp_col_vec['NOT_WEIRD_HAP'] <- 'red'
cp_col_vec['WEIRD_HAP'] <- 'blue'
cp_col_vec['QUESTIONABLE'] <- 'black'

cp_colors <- cp_hap_vec
for(cpname in names(cp_col_vec)){
  tmp_inds <- which(cp_colors == cpname)
  cp_colors[tmp_inds] <- cp_col_vec[cpname]
}
...
allsamp_combo_outfile_nolength <- '/global/cscratch1/sd/grabowsp/sg_8X_scratch/samp_filtering/all_samp_tpeds/allsamp_NJ_tree_combolabel_nolength_cp_haplotypes_v2.pdf'

pdf(file = allsamp_combo_outfile_nolength, height = 50, width = 10)
plot(combo_nj, type = 'phylogram', use.edge.length = F, no.margin = T,
  cex = 0.3, tip.color = cp_colors)
dev.off()
