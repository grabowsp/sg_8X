# Script for generating dosage-based genotype matrix from tetrasomic genotype 
#   VCF and using different types of genotypes; 
#   VCF must have header for this to work
#  
# Uses these tetrasomic dosage genotypes:
...
# [3]: (character) directory where genotype matrix will be saved
###############
...
out_suffix <- paste('_', comp_type, '_geno_dosage_mat.rds', sep = '')
...
geno_df <- data.frame(CHROM = vcf$CHROM, POS = vcf$POS, cbind(oct_df, tet_df),
  stringsAsFactors = F)

saveRDS(geno_df, file = out_file)
