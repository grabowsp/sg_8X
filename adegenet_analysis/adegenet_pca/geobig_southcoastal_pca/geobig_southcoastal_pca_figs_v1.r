...
### LOAD INPUTS ###
pca_res_file <- '/global/cscratch1/sd/grabowsp/sg_8X_scratch/pca_analysis/geobig_southcoastal_pca/GW.50kSNPs.tetrasomic.CDS.geobig_southcoastal.genlight.PCAresults.rds'
...
out_short <- 'GW.50kSNPs.geobig_southcoastal.PCA.pdf'
...
for(i in seq(length(pcs_to_plot))){
  ...
  pca_plot_list[[i]] <- ggplot(pca_df, 
    ...
    ggtitle(paste('PCA for 449 geobig SouthCoastal samps\nusing 50k SNPs, PC', 
      pcX, ' vs PC', pcY, sep = ''))
}
