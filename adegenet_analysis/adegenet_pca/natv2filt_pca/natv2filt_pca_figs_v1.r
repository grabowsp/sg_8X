...
### LOAD INPUTS ###
pca_res_file <- '/global/cscratch1/sd/grabowsp/sg_8X_scratch/pca_analysis/natv2filt_pca/GW.50kSNPs.tetrasomic.CDS.natv2filt.genlight.PCAresults.rds'
...
### SET OUTPUTS ###
out_dir <- '/global/cscratch1/sd/grabowsp/sg_8X_scratch/pca_analysis/natv2filt_pca/'
out_short <- 'GW.50kSNPs.natv2filt.PCA.pdf'
...
for(i in seq(length(pcs_to_plot))){
  ...
  pca_plot_list[[i]] <- ggplot(pca_df, 
    ...
    ggtitle(paste('PCA for 706 natural_v2 samps\nusing 50k SNPs, PC', 
      pcX, ' vs PC', pcY, sep = ''))
}
# check if see X pattern for PC4 vs PC5
pcX <- 4
pcY <- 5
#
pca_plot_list[[(i+1)]] <- ggplot(pca_df,
  aes_string(x = paste('PC', pcX, sep = ''),
  y = paste('PC', pcY, sep = ''))) +
  geom_point(aes(color = ploidy)) + ploidy_palette +
  xlab(paste('PC', pcX, ' (', round(per_var_vec[pcX], 2), '%)', sep = '')) +
...
pdf(out_full, height = 5*2.5, width = 6*2)
wrap_plots(pca_plot_list, nrow = 3)
dev.off()
...
samp_meta$STATE[which(samp_meta$VCF_NAME %in% 
  pca_df$lib[which(pca_df$PC1 < -15 & pca_df$PC5 > 10)])]
